...
def remove_all_after(items: list, border: int) -> Iterable:
    """
    Remove all elements that are located after border element.
    Leave list unchanged if border element can't be found
    """
    try:
        cut_index = items.index(border)
        return items[:cut_index + 1]
    except ValueError:
        return items
